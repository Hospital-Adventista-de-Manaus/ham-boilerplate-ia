  ...
  return (
    <main className='mx-auto flex min-h-screen max-w-3xl flex-col gap-8 px-6 py-16'>
      <header className='space-y-3'>
        <Link href='/' className='inline-block text-sm text-zinc-500 hover:text-zinc-900'>
          ← voltar
        </Link>
      ...
      <section className='rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm'>
        ...
        <p className='mb-3 text-sm text-zinc-500'>
          Chamada feita no servidor Next.js usando a rede interna (Docker / Coolify). Não passa pelo
          browser, não precisa de CORS.
        </p>
        {'error' in serverData ? (
          <div className='rounded-lg bg-red-50 p-4 text-sm text-red-700'>
            <strong>Falhou:</strong> {serverData.error}
            <p className='mt-2 text-xs text-red-600'>
              Verifique se a env <code>INTERNAL_API_URL</code> aponta para o nome de serviço correto
              da API.
            </p>
          </div>
        ) : (
      ...
      <section className='rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm'>
        ...
        <p className='mb-3 text-sm text-zinc-500'>
          Chamada feita pelo browser usando a URL pública da API. Testa CORS — se falhar, ajuste{' '}
          <code>CORS_ORIGINS</code> na API.
        </p>
